...
class DbtCloudCacheableAssetsDefinition(CacheableAssetsDefinition):
    ...
    def _build_dbt_cloud_assets_cacheable_data(
        ...
        specs = build_dbt_asset_specs(
            ...
            select=" ".join(
                f"fqn:{'.'.join(wrapped_manifest.get_node(unique_id)['fqn'])}"
        ...
        return AssetsDefinitionCacheableData(
            ...
            metadata_by_output_name={
                spec.key.to_python_identifier(): self._build_dbt_cloud_assets_metadata(
                    wrapped_manifest.get_node(spec.metadata[DAGSTER_DBT_UNIQUE_ID_METADATA_KEY])
                )
                for spec in specs
            },
            extra_metadata={
                ...
                "fqns_by_output_name": {
                    spec.key.to_python_identifier(): wrapped_manifest.get_node(
