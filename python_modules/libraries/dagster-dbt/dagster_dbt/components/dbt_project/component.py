...
class ComponentDagsterDbtTranslator(DagsterDbtTranslator):
    def __init__(self, *, resolving_info: TranslatorResolvingInfo):
        ...
        self._specs_map: dict[str, AssetSpec] = {}
    ...
    def get_asset_spec(
        ...
        if unique_id not in self._specs_map:
            base_spec = super().get_asset_spec(
                manifest=manifest,
                unique_id=unique_id,
            )
            self._specs_map[unique_id] = self.resolving_info.get_asset_spec(
                base_spec,
                {
                    self.resolving_info.obj_name: manifest.get_node(unique_id),
                    "spec": base_spec,
                },
            )

        return self._specs_map[unique_id]
