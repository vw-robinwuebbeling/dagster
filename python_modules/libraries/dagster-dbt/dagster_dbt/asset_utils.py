...
def get_asset_key_for_model(dbt_assets: Sequence[AssetsDefinition], model_name: str) -> AssetKey:
    ...
    matching_model_ids = [
        unique_id
        for unique_id, value in manifest.get_section("nodes").items()
    ...
    return dagster_dbt_translator.get_asset_spec(manifest, next(iter(matching_model_ids))).key
...
def get_asset_keys_by_output_name_for_source(
    ...
    matching = {
        unique_id: value
        for unique_id, value in manifest.get_section("sources").items()
    ...
    return {
        dagster_name_fn(value): dagster_dbt_translator.get_asset_spec(manifest, unique_id).key
...
def get_manifest_and_translator_from_dbt_assets(
    dbt_assets: Sequence[AssetsDefinition],
) -> tuple["DbtManifestWrapper", "DagsterDbtTranslator"]:
    ...
    return manifest_wrapper, dagster_dbt_translator


def get_asset_keys_to_unique_id(
    manifest: "DbtManifestWrapper",
    translator: "DagsterDbtTranslator",
) -> Mapping[AssetKey, str]:
    return {
        translator.get_asset_spec(manifest, unique_id).key: node
        for unique_id, node in manifest.get_section("nodes").items()
...
def default_asset_check_fn(
    ...
    test_resource_props = manifest.get_node(test_unique_id)
    parent_unique_ids: set[str] = set(manifest.get_section("parent_map").get(test_unique_id, []))
    ...
    additional_deps = {
        dagster_dbt_translator.get_asset_spec(manifest, parent_id).key
...
def get_upstream_unique_ids(
    ...
    for parent_unique_id in dbt_resource_props.get("depends_on", {}).get("nodes", []):
        parent_node_info = manifest.get_node(parent_unique_id)
        ...
        if is_non_asset_node(parent_node_info):
            ...
            while queue:
                ...
                candidate_parent_info = manifest.get_node(candidate_parent_id)
...
def build_dbt_specs(
    *,
    ...
    for unique_id in selected_unique_ids:
        ...
        for child_unique_id in manifest.get_section("child_map")[unique_id]:
...
def _validate_asset_keys(
    ...
    for key, unique_ids in unique_ids_by_key.items():
        ...
        if translator.settings.enable_duplicate_source_asset_keys:
            resource_types = {
                manifest.get_node(unique_id)["resource_type"] for unique_id in unique_ids
            }
            if resource_types == {"source"}:
                continue
        formatted_ids = [
            f"  - `{id}` ({manifest.get_node(id)['original_file_path']})"
...
def get_asset_check_key_for_test(
    ...
    if attached_node_ref:
        ...
        project_name = manifest.get_section("metadata")["project_name"]
        if not ref_package:
            ref_package = project_name

        attached_node_unique_id = None
        for unique_id, dbt_resource_props in manifest.get_section("nodes").items():
    ...
    return AssetCheckKey(
        name=test_resource_props["name"],
        asset_key=dagster_dbt_translator.get_asset_spec(manifest, attached_node_unique_id).key,
    )
...
def get_subset_selection_for_context(
    ...
    if excluded_checks_on_selected_assets:
        ...
        selected_dbt_resources = [
            *selected_asset_resources,
            *get_dbt_test_names_for_check_keys(
                dagster_dbt_translator, manifest, assets_def, checks_on_non_selected_assets
            ),
        ]
